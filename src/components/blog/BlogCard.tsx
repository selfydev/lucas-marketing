    ...
    <article className="h-full">
      <Card className="group h-full gap-0 overflow-hidden p-0">
        {post.coverImage?.url ? (
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-slate-900/15 to-transparent" aria-hidden="true" />
          </Link>
        ) : null}
        ...
        <CardContent className="flex h-full flex-col gap-4 p-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600">{post.excerpt || "Continue reading"}</p>
          </div>
          ...
          <div className="mt-auto flex items-center justify-between gap-4 pt-2">
            ...
            <Button asChild size="sm" variant="ghost">
              <Link params={{ slug: post.slug }} search={{ page }} to="/blog/$slug">
                Read post
              </Link>
            </Button>
          </div>

...
export function UIThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (activeColor) {
      // Guard: only update if different from current state (prevents redundant updates)
      const currentBgColor = document.body.style.backgroundColor;
      const currentThemeColor = metaThemeColor.getAttribute("content");

      if (currentBgColor !== activeColor || currentThemeColor !== activeColor) {
      ...
      if (
        currentBgColor !== originalColorsRef.current.backgroundColor ||
        currentThemeColor !== originalColorsRef.current.themeColor
      ) {
        document.body.style.backgroundColor =
          originalColorsRef.current.backgroundColor;
        metaThemeColor.setAttribute(
          "content",
          originalColorsRef.current.themeColor
        );
      }
    }
  ...
  const setOverride = (color: string | null) => {
    // CRITICAL: Safari iOS requires synchronous DOM manipulation during user interactions
    // Manipulate DOM directly first, then update React state for consistency
    let metaThemeColor = document.querySelector('meta[name="theme-color"]');
    if (!metaThemeColor) {
      metaThemeColor = document.createElement("meta");
      metaThemeColor.setAttribute("name", "theme-color");
    ...
    if (color) {
      // Apply override color immediately (synchronously)
      document.body.style.backgroundColor = color;
      metaThemeColor.setAttribute("content", color);
    } else {
      // Clear override: restore to base theme or original colors
      const restoreColor =
        baseThemeColor ?? originalColorsRef.current?.backgroundColor ?? "";
      const restoreThemeColor =
        baseThemeColor ?? originalColorsRef.current?.themeColor ?? "";

      if (restoreColor) {
        document.body.style.backgroundColor = restoreColor;
      }
      if (restoreThemeColor) {
        metaThemeColor.setAttribute("content", restoreThemeColor);
      }
    }

    // Then update React state for consistency
